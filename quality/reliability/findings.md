...
- **FIXED (commits 12–13):** R-029, R-023.
  - R-029: `finish_capture` now waits on a bounded drain barrier (the worker publishes a `worker_idle` flag when the ring is empty and all popped samples are in the accumulator) before taking the buffer, so a captured tail can no longer be stranded in the worker's in-flight batch.
  - R-023: the personalization DB calls (`list/add/delete_vocabulary`, `record_user_edit`, `get_voice_style_profile`) dropped `#[frb(sync)]`, so they run on a worker instead of the Dart UI isolate (their Dart repository was already async and callers already `await`). Settings load/save intentionally stay sync — the files are <2 KB and `load_settings` runs at synchronous view-model construction, so making it async would ripple widely for sub-millisecond work.
- **FIXED (commits 7–9):** R-018, R-010, R-030, and the practical halves of R-006/R-014/R-012.
  - R-018: audio-meter events are throttled to ~25 Hz at the emission point, bounding the Rust→Dart stream.
  - R-010: the ASR worker keeps a 2-entry MRU cache of warm Whisper contexts, so alternating languages no longer reload a ~190 MB model each switch.
  - R-030: `finish_capture` tolerates ~100 ms of dropped samples instead of discarding an utterance on a single drop.
  - R-014 / R-006: the audio worker and the preview worker now expose an `exited` flag; teardown waits a bounded 2 s then **detaches** instead of joining forever (a blocked callback / in-flight decode can no longer hang `Drop` or finish/cancel). The preview decode is already bounded by the ASR watchdog, so a detached worker self-terminates.
  - R-012 (CPU half): the audio worker batches its drain and takes the accumulator lock **once per pass** instead of ~16k times/sec, and runs level callbacks after releasing the lock. **R-012 remaining:** pausing the idle mic stream is deferred — it would disable pre-roll (cutting first syllables) and belongs to the battery-saver performance mode.
- **FIXED (commit 6 — model install/status/load path):** R-016, R-019, R-022. `recommended_model_status` (a `#[frb(sync)]` UI-thread call) no longer full-SHA-256-hashes ~190 MB of models on every Settings render — it uses a cheap presence + size check; the strong hash still runs at install and the new load-time guard (R-016). `ensure_model_file` now re-checks the model size against its expected minimum before handing it to whisper.cpp, so a later-truncated model fails cleanly with a reinstall prompt instead of risking a native abort (R-019). Model download runs through a `ureq` agent with connect/idle timeouts and guarantees the `.partial` file is removed on every error path — network, disk-full mid-write, or verification mismatch (R-022). Regression test: `models::expected_minimum_bytes_matches_each_known_model`. (Free-space precheck omitted: it needs a new dependency and disk-full already fails cleanly with cleanup.)
- **FIXED (commit 5 — storage growth & integrity):** R-017 (partial), R-021, R-024. History is now bounded by an automatic retention sweep (`enforce_history_retention`, default 365 days) run after each dictation; the learning table is capped at 5,000 rows and the vocabulary table at 1,000 (enforced on write + a `LIMIT` on the per-dictation load path), so the hot-path scan can't grow unbounded (R-024). `save_dictation` uses a plain `INSERT` instead of `INSERT OR REPLACE`, so the FTS delete trigger can no longer be skipped and orphan index content (R-021). Regression tests: `storage::enforce_retention_prunes_old_dictations_and_keeps_recent_ones` (also asserts the FTS index updates on prune), `vocabulary_is_capped_to_bound_the_hot_path`. **R-017 completed (commit 11):** a user-facing "Keep history for" control (30/90/180/365 days, default 1 year) now lives in System Settings. Added `history_retention_days` to `NativeSettings`, regenerated the flutter_rust_bridge bindings, plumbed it through the Dart domain/repository/view-model, and made `enforce_history_retention` prune using the persisted, validated value. Regression tests: Rust `settings::retention_days_falls_back_when_out_of_range`, Dart `history retention choice persists and defaults to one year`. (Visual placement is worth a final `/run` check.)
- **FIXED (commit 4 — privacy hygiene):** R-015, R-025. Captured PCM is now scrubbed rather than merely freed: the raw capture buffer is zeroed after resampling, `ActiveBuffer::drop` zeroes its samples (covering cancel and abnormal exit), and `CaptureAccumulator::drop` zeroes the rolling pre-roll (R-015). The BYOK provider key is redacted from a manual `Debug` impl so a stray `{:?}` in a log or panic message can never leak it (R-025; a `Drop`-based zeroize was intentionally avoided because frb's generated `SseEncode` moves fields out, which `Drop` would forbid — noted as a wrapper-type follow-up). Regression test: `api::dictation::debug_output_redacts_the_provider_api_key`.
